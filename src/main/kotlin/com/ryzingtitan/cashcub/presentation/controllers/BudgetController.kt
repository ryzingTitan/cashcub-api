...
class BudgetController(
    ...
    ): Budget = budgetService.create(budgetRequest)

    @PostMapping("/{budgetId}/clone")
    @ResponseStatus(HttpStatus.CREATED)
    @Tag(name = "Budgets")
    @Operation(summary = "Clone a budget")
    @SecurityRequirement(name = "jwt")
    @ApiResponses(
        value = [
            ApiResponse(
    ...
    suspend fun cloneBudget(
        @PathVariable budgetId: UUID,
        @RequestBody budgetRequest: BudgetRequest,
    ): BudgetSummary = budgetService.clone(budgetId, budgetRequest)
}
